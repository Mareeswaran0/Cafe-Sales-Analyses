...
-- Variables to track count of profit and loss records
DECLARE @ProfitCount INT = 0;
DECLARE @LossCount INT = 0;
...
BEGIN
	...
	declare @OverallprofitLoss varchar(55)

    -- Determine OverallprofitLoss based on desired counts of profit and loss records
    DECLARE @RandomValue float = RAND();
    IF @RandomValue < 0.87 AND @ProfitCount < 870 -- 87% chance of profit, limited to 870 profit records
    BEGIN
        SET @OverallprofitLoss = 'Profit';
        SET @ProfitCount = @ProfitCount + 1;
    END
    ELSE IF @LossCount < 230 -- 13% chance of loss, limited to 230 loss records
    BEGIN
        SET @OverallprofitLoss = 'Loss';
        SET @LossCount = @LossCount + 1;
    END
    ELSE
    BEGIN
        CONTINUE; -- Skip this iteration if we reach the limit of profit or loss records
    END;

    -- Insert the random data into the temporary table
    INSERT INTO #RandomData (Product, [Date], PurchasedPrice, InStock, SoldQuantity, SoldPrice, BalanceQuantity, TotalPP, TotalSP, Difference, OverallprofitLoss)
    VALUES (@Product, @Date, @PurchasedPrice, @InStock, @SoldQuantity, @SoldPrice, @InStock - @SoldQuantity, @TotalPP, @TotalSP, @Difference, @OverallprofitLoss);
...
INSERT INTO Cafe_2 (Product, [Date], PurchasedPrice, InStock, SoldQuantity, SoldPrice, BalanceQuantity, TotalPP, TotalSP, Difference, SumTotalPP, SumTotalSP, OverallprofitLoss)
...
select count(ID) from Cafe_2 where OverallProfitLoss='Profit'
select count(ID) from Cafe_2 where OverallProfitLoss='Loss' 
